...
#define CANCEL_IF(arg) if (arg){jsonRxBufferState = JSON_BUF_PROCESSED; continue;}
...
{
    if (jsonRxBufferState == JSON_BUF_PROCESSED && dmaRxBuffer[0] != '{' 
    ...
    jsonRxBuffer[jsonRxBufferUsed] = dmaRxBuffer[0];
    jsonRxBufferUsed++;
}
...
{
    HAL_UART_Transmit(&huart1, jsonRxBuffer, jsonRxBufferUsed, 0);
